...
- **🛡️ Sandbox & AI-Safe**: Pure JSON blueprints with zero `eval()` or arbitrary code execution vulnerabilities
- **🧪 Battle-Tested**: 100% test coverage with comprehensive test suite

## ⚖️ Why JSON Forgefy?

| Feature | `json-forgefy` | `jq` | `jsonata` | `lodash` (manual) |
| :--- | :---: | :---: | :---: | :---: |
| **Syntax** | Familiar MongoDB Aggregation | Custom DSL | Custom Expression | Imperative JS Code |
| **Runtime Dependencies** | **0 (Zero)** | C / WASM binary | Multiple dependencies | Multiple packages |
| **Safe for AI & Sandboxes** | **✅ Pure JSON, No eval** | ⚠️ Shell / WASM | ⚠️ Dynamic AST | ❌ Arbitrary JS execution |
| **Pure TypeScript Types** | **✅ Built-in** | ❌ None | ⚠️ Community | ⚠️ Partial |
| **Browser / Edge / Workers** | **✅ Yes (Universal)** | ⚠️ Heavy WASM | ✅ Yes | ✅ Yes |
| **Zero Setup / Pure JS** | **✅ Plug & Play** | ❌ System binary | ⚠️ Parsing overhead | ✅ Plug & Play |
...
- **🤖 LLM Context**: [llms.txt](https://raw.githubusercontent.com/DMBerlin/json-forgefy/main/llms.txt) (Standardized machine-readable reference for AI agents)
...
### 4. **AI & LLM Agent Pipelines (Safe, No-Eval Reshaping)**
Safely transform unstructured or messy LLM outputs without running dangerous `eval()` or complex AST engines:

```typescript
// Messy data extracted by an LLM web scraper or tool call
const scrapedProduct = {
  raw_title: "  Ergonomic Wireless Mouse (Black) - $49.99  ",
  ...
  inventory: { in_stock: "true", count: "120" },
  timestamp: "2026-09-21T00:00:00Z"
};

// Declarative blueprint generated by an agent or developer
const blueprint = {
  title: { $trim: "$raw_title" },
  sku: { $toUpper: { $substring: [{ $trim: "$raw_title" }, 0, 9] } },
  primaryCategory: { $arrayFirst: "$tags" },
  priceUsd: { $toNumber: "$pricing.amount" },
  finalPrice: {
    $subtract: [
      { $toNumber: "$pricing.amount" },
      { $multiply: [{ $toNumber: "$pricing.amount" }, { $toNumber: "$pricing.discount_rate" }] }
    ]
  },
  inStock: { $toBoolean: "$inventory.in_stock" },
  releaseYear: { $year: { date: "$timestamp" } }
};

// Execute safely in any sandbox or edge runtime
const cleanProduct = Forgefy.this(scrapedProduct, blueprint, { strict: true });
```
