 ...
 * This script creates a browser-compatible version from the installed npm package
 */
...
function createBrowserBundle() {
  ...
  let forgefyModule;
  try {
    forgefyModule = require('json-forgefy');
    console.log('✅ Loaded json-forgefy module from node_modules');
  } catch (e) {
    console.warn('⚠️  Could not load json-forgefy module:', e.message);
    console.log('Creating fallback implementation...');
  }

  // Create a standalone browser-compatible implementation
  const browserBundle = `// JSON Forgefy Browser Bundle
...
(function(global) {
  ...
  fs.writeFileSync(BROWSER_LIB_PATH, browserBundle);
